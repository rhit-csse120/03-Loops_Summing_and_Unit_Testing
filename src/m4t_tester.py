...
def run_test_fancy_sums_of_digits():
    ...
    runTestOnMethod(m4.fancy_sums_of_digits, testCases, "fancy_sums_of_digits")
    ...
    try:
        for k in range(numTests):
            ...
            if actualReturn == er:
                ...
                resultsToPrint += fstring.format(testCases.args[k], actualReturn, er)
            ...
            if actualPrint == ep:
                ...
                fstring += "  did not match\n"
                fstring += "  expected value: {}\n"
                resultsToPrint += fstring.format(testCases.args[k], actualPrint, ep)
            ...
            for i in range(len(testCases.args[k])):
                if testCases.args[k][i] == ea[i]:
                    ...
                    fstring += "  expected value: {}\n"
                    resultsToPrint += fstring.format(
                        testCases.args[k], testCases.args[k][i], ea[i]
                    )
...
class TestCaseCollection:
    ...
    def addTestCase(self, arguments, expectedReturn, expectedPrint, expectedArgsAfter):
